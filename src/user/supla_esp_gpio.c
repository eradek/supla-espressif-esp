...
unsigned char active_filter[INPUT_MAX_COUNT] = {0,0,0,0,0,0,0};
...
supla_esp_gpio_input_timer_cb(void *timer_arg) {
	...
	if ( input_cfg->step == 1 ) {
		...
		if ( v == active ) {
			...
			input_cfg->step = 2;
			active_filter[input_cfg->channel] = 1;
			return;
