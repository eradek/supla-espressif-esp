...
{
    ets_snprintf(buffer, buffer_size, "MK SWITCH 02");
}
...
void supla_esp_board_set_channels(TDS_SuplaDeviceChannel_C *channels, unsigned char *channel_count)
{
    *channel_count = 2;
    ...
    channels[0].Flags |= SUPLA_CHANNEL_FLAG_CHANNELSTATE|SUPLA_CHANNEL_FLAG_COUNTDOWN_TIMER_SUPPORTED;

    channels[1].Number = 1;
    channels[1].Type = SUPLA_CHANNELTYPE_RELAY;
    channels[1].FuncList = SUPLA_BIT_FUNC_POWERSWITCH | SUPLA_BIT_FUNC_LIGHTSWITCH;
    channels[1].Default = SUPLA_CHANNELFNC_POWERSWITCH;
    channels[1].value[0] = supla_esp_gpio_relay_on(B_RELAY2_PORT);
    channels[1].Flags |= SUPLA_CHANNEL_FLAG_CHANNELSTATE|SUPLA_CHANNEL_FLAG_COUNTDOWN_TIMER_SUPPORTED;
}
