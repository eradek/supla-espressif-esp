...
#define B_BTN2_PORT 5

void ICACHE_FLASH_ATTR supla_esp_board_set_device_name(char *buffer, uint8 buffer_size)
{
    ets_snprintf(buffer, buffer_size, "MK ROLLERSHUTTER 01");
}

void ICACHE_FLASH_ATTR supla_esp_board_gpio_init(void)
{
    if (supla_esp_cfg.CfgButtonType == BTN_TYPE_BISTABLE)
        supla_log(LOG_DEBUG, "BTN_TYPE_BISTABLE");
    else
        supla_log(LOG_DEBUG, "INPUT_TYPE_BTN_MONOSTABLE_RS");

    supla_input_cfg[0].type = INPUT_TYPE_BTN_MONOSTABLE;
    supla_input_cfg[0].gpio_id = B_CFG_PORT;
    supla_input_cfg[0].flags = INPUT_FLAG_PULLUP | INPUT_FLAG_CFG_BTN;

    supla_input_cfg[1].type = supla_esp_cfg.CfgButtonType == BTN_TYPE_BISTABLE ? INPUT_TYPE_BTN_BISTABLE : INPUT_TYPE_BTN_MONOSTABLE_RS;
    ...
    supla_input_cfg[2].type = supla_esp_cfg.CfgButtonType == BTN_TYPE_BISTABLE ? INPUT_TYPE_BTN_BISTABLE : INPUT_TYPE_BTN_MONOSTABLE_RS;
